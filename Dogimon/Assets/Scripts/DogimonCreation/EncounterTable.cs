...
{
	public enum DogimonNumber { DogimonNull, Corgi, EnglishBulldog, Labrador } //number in string(text)
	...
	void OnTriggerEnter2D(Collider2D collision)
	{
		if (collision.tag == "Opponent")
		{
			...
			if (opponentsEncounterNum.num == (int)EncountNum.test)
				AddToParty(dogimonIndex.dogimonOne, opponentParty, parties.opponentParty, 0, -1, 5);
